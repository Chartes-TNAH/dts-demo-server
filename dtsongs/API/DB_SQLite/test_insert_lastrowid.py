...
print("Entrez-les sous la forme: Col, Collection_parent, titre, auteur, interprète (s'il est connu), date (année), "
      "chemin vers le fichier, lien ")
line = input()
...
while True:
    ...
    cursor.execute('''INSERT INTO chant_metadata (chant_ID, Col, Collection_parent, Titre, Auteur, Interp, 
    Date_creation, Content, Link) "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", to_db)''', line).lastrowid
